...
<script>
    $(document).ready(function() {
        ...
        function load_tugas() {
            $("#tugas_head").load("{{ route('get_food') }}?", "data", function(
        ...
        $(document).on('click', '.muncul', function() {
            ...
            var status = $(".status" + id_menu).val();

            if (status == 'Show') {

                $('.mytr' + id_menu).show();
                $(".status" + id_menu).val('Hidden');
                $.ajax({
                    method: "GET",
                    url: "{{ route('get_tr2') }}?id_menu=" + id_menu,
                    dataType: "html",
                    success: function(hasil) {
                        $('.mytr' + id_menu).html(hasil);

                    }
                });
            } else {
                $(".status" + id_menu).val('Show');
                $('.mytr' + id_menu).hide();
            }
        });
        $(document).on('click', '.muncul2', function() {
            ...
            var id_menu = $(this).attr('id_menu');
            var status = $(".status2" + id_menu).val();
            if (status == 'Show') {

                $('.mytr' + id_menu).show();
                $(".status2" + id_menu).val('Hidden');
                $.ajax({
                    method: "GET",
                    url: "{{ route('get_tr3') }}?id_menu=" + id_menu,
                    dataType: "html",
                    success: function(hasil) {
                        $('.mytr' + id_menu).html(hasil);

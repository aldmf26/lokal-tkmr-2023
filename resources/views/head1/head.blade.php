...
<script>
    $(document).ready(function() {
        ...
        function load_tugas() {
            var id = $('#id_distribusi').val();
            $("#tugas_head").load("{{ route('get_meja') }}?id="+id, "data", function(
        ...
        $(document).on('click', '.muncul', function() {
            ...
            var status = $(".status" + id_meja).val();

            if (status == 'Show') {

                $('.mytr' + id_meja).show();
                $(".status" + id_meja).val('Hidden');
                $.ajax({
                    method: "GET",
                    url: "{{ route('get_tr') }}?id_meja=" + id_meja,
                    dataType: "html",
                    success: function(hasil) {
                        $('.mytr' + id_meja).html(hasil);

                    }
                });
            } else {
                $(".status" + id_meja).val('Show');
                $('.mytr' + id_meja).hide();
            }
        });
        $(document).on('click', '.muncul2', function() {
            ...
            if (status == 'Show') {
                ...
                $.ajax({
                    method: "GET",
                    url: "{{ route('get_tr4') }}?id_meja=" + id_meja,
                    dataType: "html",
                    success: function(hasil) {
                        $('.mytr' + id_meja).html(hasil);
